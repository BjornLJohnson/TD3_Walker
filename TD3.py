...
from common import ReplayBuffer, Actor, Twin_Critic, update_target_model
...
class TD3:
    def __init__(
        ...
        self.critic = Twin_Critic(self.state_dim, self.action_dim).to(self.device)
        self.critic_target = Twin_Critic(self.state_dim, self.action_dim).to(self.device)
    ...
    def update_actor(self, s):
        ...
        on_policy_Q1 = self.critic.Q1(s, on_policy_a)
    ...
    def update_critics(self, s, a, r, s_prime, not_done):
        with torch.no_grad():
            ...
            Q1_tilde, Q2_tilde = self.critic_target(s_prime, a_tilde)
        ...
        critic_loss = torch.nn.functional.mse_loss(y, Q1) + torch.nn.functional.mse_loss(y, Q2)
    ...
    def train(self, num_steps):
        ...
        for t in range(num_steps):
            ...
            self.update_critics(s_batch, a_batch, r_batch, s_prime_batch, not_done_batch)
...
if __name__ == "__main__":
    ...
    # Train the policy
    td3_object.train(int(1e6))

    # profiler.stop()
    # profiler.print()

    td3_object.visualize_rollout()
